# models/

Runtime ONNX inference models.

Expected layout at runtime:

```
models/
   tooltip.onnx                    YOLOX-Nano tooltip-region detector
   tooltip.LICENSE.txt             Detector license and provenance
   paddle/
      <family>/
         rec.onnx                  PaddleOCR recognizer (per script family)
         dict.txt                  Per-recognizer character dictionary
      en/
         rec_font.onnx             Optional SaintKDG/Pelagiad recognizer
         font_dict.txt             Printable-ASCII dictionary for rec_font
```

For English, GrimVault prefers `rec_font.onnx` when both optional font-model
files are present and falls back to the stock `rec.onnx` otherwise. The font
model is generated by `tools/ocr-train`; its source fonts remain outside this
public repository pending an explicit redistribution-license review.

The tooltip detector is committed and reproducible with `tools/tooltip-train`.

Where to get the OCR models:

1. **Existing Electron build** — copy from `src/native/.build/models/` if you have a working v1.x checkout.
2. **DarkerDB releases** — download the `models-vX.Y.Z.zip` artifact attached to any GrimVault GitHub Release and extract here.

Without these files, the tooltip detector and OCR pipeline log init failures at startup; the main UI still launches and the rest of the app remains usable. See `DEV.md` for the full dev-mode setup.
