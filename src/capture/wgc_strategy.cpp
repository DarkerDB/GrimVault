...
namespace {
   ...
   constexpr int  frame_pool_buffers  = 3;
   constexpr auto first_frame_timeout = std::chrono::milliseconds (500);
...
struct WgcStrategy::Impl
{
   ...
   bool session_matches (void* target, bool is_window) const noexcept
   {
      return cont_active.load (std::memory_order_relaxed)
         && cont_target == target
         && cont_is_window == is_window;
   }
};
...
core::Result<Frame> WgcStrategy::capture_window (void* window)
{
   ...
   if (!impl_->session_matches (window, true)) {
      if (auto started = start_continuous (window, true); !started.has_value ()) {
         return core::fail (started.error ());
      }
   }

   return latest_frame (first_frame_timeout);
}
...
core::Result<Frame> WgcStrategy::capture_monitor (void* monitor)
{
   ...
   if (!impl_->session_matches (hmon, false)) {
      if (auto started = start_continuous (hmon, false); !started.has_value ()) {
         return core::fail (started.error ());
      }
   }
...
core::Result<Frame> WgcStrategy::latest_frame (std::chrono::milliseconds timeout)
{
   ...
   while (true) {
      Direct3D11CaptureFrame captured { nullptr };
      {
         ...
         if (!impl_->cont_cv.wait_until (lk, deadline, [this] {
