...
#
#    tools/fetch-ocr-model.sh latin
...
for family in "$@"; do
   ...
   echo "==> $family  ($repo)"
   curl -sfL "https://huggingface.co/$repo/resolve/main/inference.onnx" -o "$dest/rec.onnx"
...
model = onnx.load(f'{dest}/rec.onnx')
simplified, ok = simplify(model, overwrite_input_shapes={'x': [1, 3, 48, 320]})
assert ok, 'onnxsim check failed'
onnx.save(simplified, f'{dest}/rec.onnx')

print(f'    dict: {len(chars)} chars; model simplified to 1x3x48x320')
PY
done
